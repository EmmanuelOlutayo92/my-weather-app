...
export default defineEventHandler(async (event) => {
  ...
  const countryCode = (query.country as string) || "GB";

  const config = useRuntimeConfig();
  const { apiKey, baseAddress } = config.openWeather;
  let result: WeatherResultMapping;
  ...
  if (!apiKey) {
    throw createError({
      statusCode: 401,
      statusMessage:
        "OpenWeather API key is not configured on the server. Unauthorised Access",
    });
  }
  if (!city?.trim()) {
    throw createError({
  ...
  try {
    const url = `${baseAddress}/weather?q=${encodeURIComponent(
      city
